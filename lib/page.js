...
class Page {
  ...
  _makeParser() {
    ...
    const options = this.inputs.map((input) => ({
      option: input.key,
  ...
  _getCLIOptions(argv) {
    if (!this.parser) this.parser = this._makeParser();
    const options = this.parser.parseArgv(argv);
