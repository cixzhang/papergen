...
const toPixelsFromMM = (dimensionsInMM, ppi) => {
  if (!ppi) ppi = PPI;
  return dimensionsInMM.map(v => v/MM_PER_INCH * ppi);
};
...
const getSizeWithOrientation = (keyOrDimension, landscape) => {
  let sizeInMM = getSize(keyOrDimension);

  if (landscape) {
    sizeInMM = toLandscape(sizeInMM);
  } else {
    sizeInMM = toPortrait(sizeInMM);
  }
  return sizeInMM;
};
